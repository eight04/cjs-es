...
function createHoistImportTransformer({s, topLevel, scope, code}) {
  ...
  function write({excludeTopLevel = false} = {}) {
    for (const imported of imports.values()) {
      ...
      const requires = excludeTopLevel ? 
        imported.requires.filter(r => !r.node.topLevelImport) : imported.requires;
  ...
  function transform(node) {
    if (node.bareImport || scope.has("require")) {
      return;
    }
    ...
    if (!required.node.topLevelImport && imported.nonTopLevelImportPos === null) {
