...
const {getRequireInfo, hasDefaultComment, pathToName} = require("./util");

function createHoistExportTransformer({s, topLevel, scope, exportStyle}) {
  ...
  function write() {
    if (
      shouldHoist ||
      hasNamedExports() && exportStyle() === "default"
    ) {
      writeDeclare();
      writeExport();
      isTouched = true;
  ...
  function hasNamedExports() {
    return exportNodes.length > 0 ||
      moduleNodes.some(n => n.isNamedExport);
  }
...
function createHoistImportTransformer({s, topLevel, scope, code, importStyle}) {
  ...
  function transform(node) {
    ...
    if (!imported) {
      imported = {
        ...
        preferDefault: hasDefaultComment(code, required) || importStyle(required.value) === "default",
        requires: [],
