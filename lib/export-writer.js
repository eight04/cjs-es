function createExportWriter(context) {
  ...
  function write() {
    ...
    if (
      ...
      context.defaultExports.length === 1 && (
        // declared export `const foo = module.exports = ...`
        context.defaultExports[0].declared ||
        // export is referenced somewhere?
        !context.defaultExports[0].exported ||
        // export is not top-level
        !context.defaultExports[0].exported.statement ||
        // export value it not object literal
        !context.defaultExports[0].exported.object ||
        // some keys are computed or strings
        context.defaultExports[0].exported.object.properties.some(
          p => p.computed || p.key.type === "Literal") ||
        // some values are functions containing this
        context.defaultExports[0].exported.object.properties.some(
          p => p.value.containThis) ||
        // some names are not exported in the object
        !listContains(
          context.defaultExports[0].exported.object.properties.map(p => p.key.name),
          context.namedExports.keys()
        ) ||
        // default comment
  ...
  function listContains(list, items) {
    list = new Set(list);
  ...
  function writeProperty({
    ...
    } else if ((property.value.type === "Identifier" || property.required)) {
      // foo: bar
      context.s.overwrite(
        start,
        property.value.start,
  ...
  function bindToNames() {
    ...
    if (context.defaultExports.length) {
      ...
      for (let i = 0; i < properties.length; i++) {
        ...
        let infos = context.namedExports.get(name);
        if (!infos) {
          infos = [];
          context.namedExports.set(name, infos);
        }
        infos.push(nameInfo);
        start = properties[i].value.end;
      }
      extraTrims.push([start, node.exported.statement.end]);
    }
    
    // names
    for (const [name, infos] of context.namedExports) {
      let init = 0;
      let assignment = 0;
      let declared = 0;
    ...
    for (const [start, end] of extraTrims) {
