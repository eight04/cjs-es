...
function createAnalyzer(context) {
  ...
  function analyzeCallable(node) {
    if (
      ...
      context.callableIds.add(node.callee.name);
    } else if (node.callee.type === "CallExpression") {
      analyzeRequire(node.callee);
      const required = node.callee.topRequired || node.callee.required;
      if (required) {
        required.callable = true;
        // FIXME: bad design, context.skip is called inside analyzeRequire.
      }
    }
  }
  ...
  function analyzeReassign(node) {
    ...
    if (
      ...
      context.callableIds.add(left.name);
    }
  }
