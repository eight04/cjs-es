    ...
    if (exported.isNamed) {
      if (exported.preferDefault) {
        if (hoist) {
          ...
          throw new Error("Can not collect named exports to a single object without hoist transformer");
        }
      }
      ...
      const overwrite = (start, property, newLine, semi) => {
        if (property.value.type === "Identifier") {
          ...
          // foo: require("...")
          const preferDefault = hasDefaultComment(code, property.required) ||
            importStyle(property.required.value) === "default";
          s.prependRight(
            node.start,
            `import ${preferDefault ? "" : "* as "}_export_${property.name}_ from ${JSON.stringify(property.required.value)};\n`
          );
          s.overwrite(
            start,
            property.key.start,
