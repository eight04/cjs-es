...
If you already have the AST of the code, you can set it as `ast` so the module don't have to parse the code again.

If `sourceMap` is `true` then generate the source map.
...
The transformer will use `warn` function to emit a warning. If `warn` is not set then the transformer will print the message to the console using `console.error`.
...
The output ES source will be saved as `code`.

If `isTouched` is `true` then the code is changed.
